...
public class RoomBackGroundCtrl : MonoBehaviour
{
    enum RoomVideoState
    {
        first,
    ...
    void Update()
    {
        ...
        if (PlayTime <= 0.0f && roomVideoState == RoomVideoState.first)
        {
            roomVideoState = RoomVideoState.Second;
            PlayTime = VideoTime - rootStartTime;
        }
        else if (PlayTime <= 0.0f && roomVideoState == RoomVideoState.Second)
        {
            isSecondStart = false;
            PlayTime = VideoTime - rootStartTime;
        }
    ...
    IEnumerator PrepareVideo()
    {
        ...
        {
            if (roomVideoState == RoomVideoState.Second)
            {
